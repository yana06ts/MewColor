...
export function PixelGrid({
  ...
  return (
    <div className="flex flex-col h-full bg-creamPuff select-none">
      ...
      {/* 2. Visual Powerups/Skills Row */}
      <div className="flex justify-around items-center px-4 py-2.5 bg-white/50 border-b border-rose-100/40 shadow-xs">
        {/* Magic Wand powerup */}
        <button
          id="powerup-wand-btn"
          onClick={() => {
            if (activeSpecialTool === "wand") {
              setActiveSpecialTool("pencil");
            } else if (powerupCounts.wand > 0) {
              setToolMode("draw");
              setActiveSpecialTool("wand");
            } else {
              SOUNDS.playError();
            }
          }}
          className={`flex flex-col items-center p-1.5 px-3 rounded-xl border transition-all ${
            activeSpecialTool === "wand"
              ? "bg-indigo-100 border-indigo-400 text-indigo-700 scale-102"
              : "bg-white border-rose-100 text-slate-600 hover:bg-rose-50"
          }`}
        >
          <div className="flex items-center gap-1">
            <Sparkles className="w-4 h-4 text-indigo-500" />
            <span className="text-xs font-bold font-pixel scale-85">🪄 Палочка</span>
          </div>
          <span className="text-[10px] text-indigo-600 font-pixel mt-0.5 scale-75 opacity-80">
            {powerupCounts.wand} шт
          </span>
        </button>
        ...
        <button
          ...
          onClick={() => {
            if (activeSpecialTool === "bomb") {
              setActiveSpecialTool("pencil");
            } else if (powerupCounts.bomb > 0) {
              setToolMode("draw");
              setActiveSpecialTool("bomb");
            } else {
              SOUNDS.playError();
            }
          }}
          className={`flex flex-col items-center p-1.5 px-3 rounded-xl border transition-all ${
            activeSpecialTool === "bomb"
              ? "bg-amber-100 border-amber-400 text-amber-700 scale-102"
              : "bg-white border-rose-100 text-slate-600 hover:bg-rose-50"
          }`}
        >
          <div className="flex items-center gap-1">
            <Bomb className="w-4 h-4 text-amber-600" />
            <span className="text-xs font-bold font-pixel scale-85">💣 Бомбочка</span>
          </div>
          <span className="text-[10px] text-amber-600 font-pixel mt-0.5 scale-75 opacity-80">
            {powerupCounts.bomb} шт
          </span>
        </button>
      </div>

      {/* Helper text based on active custom option */}
      {activeSpecialTool !== "pencil" && (
        <div className="bg-indigo-50 border-b border-indigo-100/50 text-center py-1 flex items-center justify-center gap-1 duration-150">
          <Sparkle className="w-3.5 h-3.5 text-indigo-500 animate-pulse" />
          <span className="text-xs text-indigo-700 font-semibold">
            {activeSpecialTool === "wand"
              ? "Палочка готова! Нажми на любой пиксель, чтобы раскрасить его узор"
              : "Бомбочка готова! Нажми на пиксель выбранного цвета, чтобы взорвать 3х3"}
          </span>
        </div>
      )}
